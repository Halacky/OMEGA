"""
Experiment 120: Learnable Fourier Filterbank with FULL Spectrum (5-1000 Hz)

Hypothesis:
    Experiment 115 used max_freq=500 Hz (H1 cutoff). Experiment 119 showed
    that full-spectrum MFCC gives +4.5pp for CNN (43.72% vs 39.22%).

    Learnable Fourier filters are even better positioned to exploit the full
    spectrum: they can LEARN which frequencies above 500 Hz are useful and
    which are subject noise — no hand-crafted cutoff needed.

    Additionally, exp_115 ran on E1 only. This experiment uses E1+E2 for
    fair comparison with exp_119 (MFCC fullband, 43.72%).

What is tested:
    Three configurations (same K/M as exp_115, but max_freq=1000):
      A) Fourier FB (K=32, M=8)  + PCEN + CNN-GRU  — main config
      B) Fourier FB (K=16, M=12) + PCEN + CNN-GRU  — best from exp_115
      C) Fourier FB (K=32, M=4)  + PCEN + CNN-GRU  — fewer harmonics

    Direct comparison:
      - exp_115 B (E1, fmax=500):  34.80%
      - exp_119 B (E1+E2, MFCC fmax=1000): 43.72%  ← target to beat

Run examples:
    python experiments/exp_120_fourier_fullband_loso.py --ci
    python experiments/exp_120_fourier_fullband_loso.py --ci --config B
    python experiments/exp_120_fourier_fullband_loso.py --full
"""
...
EXPERIMENT_NAME = "exp_120_fourier_fullband"
EXERCISES       = ["E1"]    # ← E1+E2 for fair comparison with exp_119
USE_IMPROVED    = True
MAX_GESTURES    = 10
...
CONFIGS = {
    "A": {
        ...
        "label": "Fourier K=32 M=8 fmax=1000 + PCEN + CNN-GRU",
    },
    "B": {
        "num_filters": 16, "num_harmonics": 12, "kernel_size": 51,
        "label": "Fourier K=16 M=12 fmax=1000 + PCEN + CNN-GRU",
    },
    "C": {
        "num_filters": 32, "num_harmonics": 4,  "kernel_size": 51,
        "label": "Fourier K=32 M=4 fmax=1000 + PCEN + CNN-GRU",
    },
}

MIN_FREQ = 5.0
MAX_FREQ = 999.0     # ← FULL NYQUIST (was 500.0 in exp_115)
PCEN_EMA_LENGTH = 128
...
def main():
    ...
    try:
        ...
        if best_config is not None:
            ...
            mark_hypothesis_verified("H_FOURIER_FULLBAND", metrics,
                                     experiment_name=EXPERIMENT_NAME)
        else:
            mark_hypothesis_failed("H_FOURIER_FULLBAND", "All configurations failed")
    except ImportError:
        pass
