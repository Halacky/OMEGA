"""
Experiment 56: Test-Time Training with Masked Channel Reconstruction (LOSO)

Hypothesis (h-056-test-time-training):
    Performing test-time training (TTT) via self-supervised masked channel
    reconstruction adapts the shared encoder to the test subject's EMG
    distribution WITHOUT using any labels. This reduces cross-subject
    domain gap and improves LOSO accuracy.

Technique:
    1. Train: L = L_gesture + lambda_ssl * L_ssl (masked channel MSE)
    2. Test-time: save encoder -> N SGD steps on L_ssl using test windows
       (no labels) -> classify with adapted encoder -> restore encoder

Architecture: TTTContentStyleEMG (models/ttt_content_style_emg.py)
"""
...
from models.ttt_content_style_emg import TTTContentStyleEMG
from models import register_model

register_model("ttt_content_style_emg", TTTContentStyleEMG)

EXPERIMENT_NAME = "exp_56_test_time_training_masked_channel_ssl_loso"
HYPOTHESIS_ID = "h-056-test-time-training"
HIDDEN_DIM = 128
MASK_RATIO = 0.25
LAMBDA_SSL = 0.5
TTT_STEPS = 10
...
class TTTTrainer(WindowClassifierTrainer):
    """
    Trainer for TTTContentStyleEMG.

    fit(): Train with L_gesture + lambda_ssl * L_ssl.
    evaluate_numpy(): TTT adaptation before classification.
    """

    def __init__(self, train_cfg, logger, output_dir, visualizer=None,
                 hidden_dim=128, mask_ratio=0.25, lambda_ssl=0.5,
                 ttt_steps=10, ttt_lr=1e-4):
    ...
    def fit(self, splits: Dict) -> Dict:
        ...
        self.logger.info(f"TTT: in_ch={in_channels}, classes={num_classes}, "
                         f"mask_ratio={self.mask_ratio}, lambda_ssl={self.lambda_ssl}, "
                         f"ttt_steps={self.ttt_steps}, ttt_lr={self.ttt_lr}")
        ...
        model = TTTContentStyleEMG(
            in_channels=in_channels, num_classes=num_classes,
            dropout=self.cfg.dropout, hidden_dim=self.hidden_dim,
            mask_ratio=self.mask_ratio,
        ).to(self.cfg.device)

        self.logger.info(f"TTTContentStyleEMG: {sum(p.numel() for p in model.parameters()):,} params")
        ...
        for epoch in range(1, self.cfg.epochs + 1):
            model.train()
            ep_total, ep_gest, ep_ssl = 0.0, 0.0, 0.0
            ...
            for xb, yb in dl_train:
                ...
                L_gesture = gesture_criterion(out["logits"], yb)
                L_ssl = model.ssl_loss(xb)
                loss = L_gesture + self.lambda_ssl * L_ssl
            ...
            if dl_val is not None:
                ...
                with torch.no_grad():
                    for xb, yb in dl_val:
                        xb, yb = xb.to(self.cfg.device), yb.to(self.cfg.device)
                        logits = model(xb)
                        vl += gesture_criterion(logits, yb).item() * yb.size(0)
    ...
    def evaluate_numpy(self, X, y, split_name="custom", visualize=False):
        ...
        # === Test-Time Training ===
        # Save encoder + SSL head state
        encoder_state = copy.deepcopy(self.model.encoder.state_dict())
        ssl_state = copy.deepcopy(self.model.ssl_recon_head.state_dict())

        # Freeze classifier, enable encoder gradients
        for p in self.model.get_classifier_params():
            p.requires_grad = False
        for p in self.model.get_encoder_params():
            p.requires_grad = True

        ttt_optimizer = optim.SGD(
            self.model.get_encoder_params(), lr=self.ttt_lr, momentum=0.9,
        )

        self.model.train()
        ttt_batch_size = min(256, len(X_tensor))

        for step in range(self.ttt_steps):
            perm = torch.randperm(len(X_tensor), device=device)
            batch_idx = perm[:ttt_batch_size]
            x_batch = X_tensor[batch_idx]

            ttt_optimizer.zero_grad()
            ssl_loss = self.model.ssl_loss(x_batch)
            ssl_loss.backward()
            torch.nn.utils.clip_grad_norm_(self.model.get_encoder_params(), max_norm=5.0)
            ttt_optimizer.step()

        # === Classify with adapted encoder ===
        self.model.eval()
        all_logits = []
        ...
        self.model.ssl_recon_head.load_state_dict(ssl_state)
...
def run_single_loso_fold(base_dir, output_dir, train_subjects, test_subject,
    ...
    train_cfg.model_type = "ttt_content_style_emg"
    ...
    trainer = TTTTrainer(
        ...
        hidden_dim=HIDDEN_DIM, mask_ratio=MASK_RATIO,
        lambda_ssl=LAMBDA_SSL, ttt_steps=TTT_STEPS, ttt_lr=TTT_LR,
    )
    ...
    try:
        ...
        traceback.print_exc()
        return {"test_subject": test_subject, "model_type": "ttt_content_style_emg",
    ...
    print(f"[LOSO-TTT] Test {test_subject} | Acc={test_acc:.4f}, F1={test_f1:.4f}")
    ...
    saver.save_metadata(make_json_serializable({
        ...
        "model_type": "ttt_content_style_emg", "exercises": exercises,
        "ttt_config": {"steps": TTT_STEPS, "lr": TTT_LR, "mask_ratio": MASK_RATIO,
...
def main():
    ...
    proc_cfg = ProcessingConfig(window_size=400, window_overlap=200, num_channels=12,
    ...
    train_cfg = TrainingConfig(
        batch_size=64, epochs=60, learning_rate=1e-3, weight_decay=1e-4,
        dropout=0.3, early_stopping_patience=12, use_class_weights=True,
        seed=42, num_workers=4,
        device="cuda" if torch.cuda.is_available() else "cpu",
        use_handcrafted_features=False, pipeline_type="deep_raw",
        model_type="ttt_content_style_emg",
    ...
    print(f"Model: TTTContentStyleEMG, mask_ratio={MASK_RATIO}, lambda_ssl={LAMBDA_SSL}")
    print(f"TTT: {TTT_STEPS} SGD steps at lr={TTT_LR}")
    print(f"Subjects ({len(ALL_SUBJECTS)}): {ALL_SUBJECTS}")

    all_results = []
    for test_subject in ALL_SUBJECTS:
        train_subjects = [s for s in ALL_SUBJECTS if s != test_subject]
        fold_dir = OUTPUT_DIR / "ttt_content_style_emg" / f"test_{test_subject}"
    ...
    if valid:
        ...
        aggregate["ttt_content_style_emg"] = {
        ...
        print(f"\nTTT: Acc={np.mean(accs):.4f}+/-{np.std(accs):.4f}, "
              f"F1={np.mean(f1s):.4f}+/-{np.std(f1s):.4f}")

    summary = {
        "experiment_name": EXPERIMENT_NAME, "hypothesis_id": HYPOTHESIS_ID,
        "feature_set": "deep_raw", "model": "ttt_content_style_emg",
        "ttt_config": {"hidden_dim": HIDDEN_DIM, "mask_ratio": MASK_RATIO,
                       "lambda_ssl": LAMBDA_SSL, "ttt_steps": TTT_STEPS, "ttt_lr": TTT_LR},
    ...
    try:
        ...
        if aggregate:
            mark_hypothesis_verified(HYPOTHESIS_ID, metrics=aggregate["ttt_content_style_emg"],
