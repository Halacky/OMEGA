"""
Experiment 50: Subject-Conditional Normalizing Flows for Canonical EMG (LOSO)

Hypothesis (h-050-normalizing-flows):
    Normalizing flows can learn invertible mappings from subject-specific
    EMG latent distributions to a shared canonical space. By training a
    CNN-GRU encoder jointly with affine coupling flow layers and a gesture
    classifier on the canonical representation, the flow learns to remove
    subject-specific distributional differences.

Architecture: FlowCanonicalEMG (models/flow_canonical_emg.py)
Training:     L_gesture + lambda_flow * L_flow (NLL under standard normal prior)
Augmentation: noise + time_warp (on-the-fly)
"""
...
from models.flow_canonical_emg import FlowCanonicalEMG
from models import register_model

register_model("flow_canonical_emg", FlowCanonicalEMG)

EXPERIMENT_NAME = "exp_50_subject_conditional_normalizing_flows_loso"
HYPOTHESIS_ID = "h-050-normalizing-flows"
LAMBDA_FLOW = 0.1
N_FLOW_LAYERS = 4
HIDDEN_DIM = 128


class FlowTrainer(WindowClassifierTrainer):
    """Custom trainer for FlowCanonicalEMG with flow NLL loss."""

    def __init__(self, train_cfg, logger, output_dir, visualizer=None,
                 lambda_flow=0.1, n_flow_layers=4, hidden_dim=128):
    ...
    def fit(self, splits: Dict) -> Dict:
        ...
        self.logger.info(f"FlowTrainer: in_ch={in_channels}, classes={num_classes}, "
                         f"flow_layers={self.n_flow_layers}, lambda_flow={self.lambda_flow}")
        ...
        self.logger.info(f"FlowCanonicalEMG: {total_params:,} parameters")
        ...
        for epoch in range(1, self.cfg.epochs + 1):
            model.train()
            ep_total, ep_gesture, ep_flow = 0.0, 0.0, 0.0
            ...
            for xb, yb in dl_train:
                ...
                L_flow = model.flow_loss(out["z"])
                loss = L_gesture + self.lambda_flow * L_flow
                ...
                ep_gesture += L_gesture.item() * bs
                ep_flow += L_flow.item() * bs
            ...
            history["gesture_loss"].append(ep_gesture / max(1, ep_count))
            history["flow_loss"].append(ep_flow / max(1, ep_count))

            self.logger.info(
                f"[Epoch {epoch:02d}/{self.cfg.epochs}] "
                f"Train: loss={train_loss:.4f} (gest={history['gesture_loss'][-1]:.4f}, "
                f"flow={history['flow_loss'][-1]:.4f}), acc={train_acc:.3f} | "
        ...
        torch.save({
            ...
            "class_ids": class_ids, "mean": mean_c, "std": std_c,
            "hidden_dim": self.hidden_dim, "n_flow_layers": self.n_flow_layers,
        }, self.output_dir / "flow_model.pt")
...
def run_single_loso_fold(base_dir, output_dir, train_subjects, test_subject,
    ...
    train_cfg.model_type = "flow_canonical_emg"
    ...
    trainer = FlowTrainer(
        ...
        lambda_flow=LAMBDA_FLOW, n_flow_layers=N_FLOW_LAYERS, hidden_dim=HIDDEN_DIM,
    )
    ...
    try:
        ...
        traceback.print_exc()
        return {"test_subject": test_subject, "model_type": "flow_canonical_emg",
    ...
    saver.save_metadata(make_json_serializable({
        ...
        "model_type": "flow_canonical_emg", "exercises": exercises,
...
def main():
    ...
    print(f"Model: FlowCanonicalEMG, flow_layers={N_FLOW_LAYERS}, lambda_flow={LAMBDA_FLOW}")
    print(f"Subjects ({len(ALL_SUBJECTS)}): {ALL_SUBJECTS}")

    all_results = []
    for test_subject in ALL_SUBJECTS:
        train_subjects = [s for s in ALL_SUBJECTS if s != test_subject]
        fold_dir = OUTPUT_DIR / "flow_canonical_emg" / f"test_{test_subject}"
    ...
    if valid:
        ...
        print(f"\nFlow Canonical: Acc={np.mean(accs):.4f}+/-{np.std(accs):.4f}, "
              f"F1={np.mean(f1s):.4f}+/-{np.std(f1s):.4f}")

    summary = {
        "experiment_name": EXPERIMENT_NAME, "hypothesis_id": HYPOTHESIS_ID,
        "feature_set": "deep_raw", "model": "flow_canonical_emg",
        "lambda_flow": LAMBDA_FLOW, "n_flow_layers": N_FLOW_LAYERS,
    ...
    try:
        ...
        if aggregate:
            best = aggregate["flow_canonical_emg"]
            mark_hypothesis_verified(HYPOTHESIS_ID, metrics=best, experiment_name=EXPERIMENT_NAME)
        else:
