"""
Experiment 54: Multi-Resolution Temporal Consensus (LOSO)

Hypothesis (h-054-multi-resolution-consensus):
    Processing EMG at 4 temporal resolutions (original, 2x, 4x, 8x
    downsampled) with independent CNN encoders, fusing via cross-resolution
    attention with a consensus regulariser (KL between per-resolution and
    ensemble predictions), yields more robust cross-subject classification.

Architecture: MultiResolutionEMG (models/multi_resolution_emg.py)
Training:     L_gesture + consensus_weight * L_consensus
...
from models.multi_resolution_emg import MultiResolutionEMG
from models import register_model

register_model("multi_resolution_emg", MultiResolutionEMG)

EXPERIMENT_NAME = "exp_54_multi_resolution_temporal_consensus_loso"
HYPOTHESIS_ID = "h-054-multi-resolution-consensus"
CONSENSUS_WEIGHT = 0.3
HIDDEN_DIM = 128


class MultiResolutionTrainer(WindowClassifierTrainer):
    """Custom trainer for MultiResolutionEMG with consensus loss."""

    def __init__(self, train_cfg, logger, output_dir, visualizer=None,
                 consensus_weight=0.3, hidden_dim=128):
    ...
    def fit(self, splits: Dict) -> Dict:
        ...
        self.logger.info(f"MultiResolution: in_ch={in_channels}, classes={num_classes}, "
                         f"consensus_weight={self.consensus_weight}")
        ...
        model = MultiResolutionEMG(
            in_channels=in_channels, num_classes=num_classes,
            dropout=self.cfg.dropout, hidden_dim=self.hidden_dim,
        ).to(self.cfg.device)

        self.logger.info(f"MultiResolutionEMG: {sum(p.numel() for p in model.parameters()):,} params")
        ...
        for epoch in range(1, self.cfg.epochs + 1):
            ...
            for xb, yb in dl_train:
                ...
                consensus_loss = model.get_consensus_loss(logits)
                loss = cls_loss + self.consensus_weight * consensus_loss
        ...
        }, self.output_dir / "multi_resolution_model.pt")
...
def run_single_loso_fold(base_dir, output_dir, train_subjects, test_subject,
    ...
    train_cfg.model_type = "multi_resolution_emg"
    ...
    trainer = MultiResolutionTrainer(
        ...
        consensus_weight=CONSENSUS_WEIGHT, hidden_dim=HIDDEN_DIM,
    )
    ...
    try:
        ...
        traceback.print_exc()
        return {"test_subject": test_subject, "model_type": "multi_resolution_emg",
    ...
    saver.save_metadata(make_json_serializable({
        ...
        "model_type": "multi_resolution_emg", "exercises": exercises,
...
def main():
    ...
    train_cfg = TrainingConfig(
        ...
        model_type="multi_resolution_emg",
    ...
    print(f"Model: MultiResolutionEMG, consensus_weight={CONSENSUS_WEIGHT}")
    print(f"Subjects ({len(ALL_SUBJECTS)}): {ALL_SUBJECTS}")

    all_results = []
    for test_subject in ALL_SUBJECTS:
        train_subjects = [s for s in ALL_SUBJECTS if s != test_subject]
        fold_dir = OUTPUT_DIR / "multi_resolution_emg" / f"test_{test_subject}"
    ...
    if valid:
        ...
        aggregate["multi_resolution_emg"] = {
        ...
        print(f"\nMultiRes: Acc={np.mean(accs):.4f}+/-{np.std(accs):.4f}, "
              f"F1={np.mean(f1s):.4f}+/-{np.std(f1s):.4f}")

    summary = {
        "experiment_name": EXPERIMENT_NAME, "hypothesis_id": HYPOTHESIS_ID,
        "feature_set": "deep_raw", "model": "multi_resolution_emg",
    ...
    try:
        ...
        if aggregate:
            mark_hypothesis_verified(HYPOTHESIS_ID, metrics=aggregate["multi_resolution_emg"],
