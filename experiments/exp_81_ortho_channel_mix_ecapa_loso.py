"""
Experiment 81: Orthogonal Channel-Mixing Augmentation for ECAPA-TDNN (LOSO)

Hypothesis
──────────
Inter-subject electrode variability can be modelled as a soft linear mixture
of neighbouring muscles rather than an arbitrary channel permutation.
SetTransformer-style random permutation destroys anatomical channel ordering
and hurts performance.  Instead, training with random orthogonal matrices
close to the identity — A = exp(εS), S skew-symmetric — simulates gentle
"electrode drift" between subjects.

For each training batch X ∈ ℝ^{B×C×T}:

    S  = ε · (R − Rᵀ) / 2   (R ~ N(0,1))   # skew-symmetric
    A  = matrix_exp(S) ∈ SO(C)               # orthogonal, det = ±1
    X' = A ⊗ X                               # channel mixing per time step

Properties:
  • ‖A − I‖_F = O(ε) → channels softly blended, not permuted (ε = 0.1)
  • ‖X'‖_2 = ‖X‖_2 → no amplitude distortion
  • A has no trainable parameters — sampled fresh each batch
  • Augmentation gated by model.training → zero test-time cost

Architecture: ECAPATDNNEmg (Res2Net + SE + Attentive Stats Pooling)
  • Unchanged from exp_62 — same parameter count ≈ 467 K
  • Augmentation lives in OrthoMixECAPAEmg wrapper, transparent to backbone
...
For each fold (test_subject ∈ ALL_SUBJECTS):
  ...
  1. Load windows for ALL subjects (train + test) in one call.
     (Test subject included only for get_common_gestures() — no signal stats.)
  ...
  5. Train OrthoMixECAPAEmg end-to-end on train split.
     model.train() → augmentation ON  (only for train batches)
     model.eval()  → augmentation OFF (for val, test, and inference)
...
Data-leakage guards
  ...
  ✓ mean_c / std_c computed inside OrthoMixECAPATrainer.fit() from X_train.
  ✓ Augmentation matrix A sampled from N(0,1) — no subject statistics.
  ✓ model.eval() at inference: BatchNorm frozen, augmentation disabled.
  ✓ evaluate_numpy() calls model.eval() — augmentation cannot leak.

Comparison to exp_62 (ECAPA-TDNN baseline)
  exp_62: ECAPATDNNEmg, no augmentation
  exp_81: OrthoMixECAPAEmg = ECAPATDNNEmg + orthogonal mixing (train only)
  Same training config, same data pipeline, same LOSO protocol.

Run examples
────────────
  # Fast CI run (5 subjects, safe server default):
  python experiments/exp_81_ortho_channel_mix_ecapa_loso.py --ci

  # Specific subjects:
  python experiments/exp_81_ortho_channel_mix_ecapa_loso.py \\
      --subjects DB2_s1,DB2_s12,DB2_s15

  # Full 20-subject run:
  python experiments/exp_81_ortho_channel_mix_ecapa_loso.py --full

  # Custom mixing strength:
  python experiments/exp_81_ortho_channel_mix_ecapa_loso.py --ci \\
      --mix-epsilon 0.05 --mix-prob 0.8
"""
...
from training.ortho_mix_ecapa_trainer import OrthoMixECAPATrainer
...
EXPERIMENT_NAME = "exp_81_ortho_channel_mix_ecapa"
APPROACH        = "deep_raw"
EXERCISES       = ["E1", "E2"]
USE_IMPROVED    = True
MAX_GESTURES    = 10

# ECAPA-TDNN architecture hyper-parameters (same as exp_62)
ECAPA_CHANNELS      = 128
ECAPA_SCALE         = 4
...
# Orthogonal mixing hyper-parameters
# mix_epsilon controls ‖A − I‖_F = O(ε).  Smaller → closer to identity.
# mix_prob: fraction of batches that are augmented.
MIX_EPSILON = 0.1
MIX_PROB    = 0.7
...
def _build_splits(
    ...
    LOSO invariants upheld here:
      ...
      ─ No signal statistics (mean, std) are computed in this function;
        normalisation happens later inside OrthoMixECAPATrainer.fit().
...
def run_single_loso_fold(
    ...
    train_cfg.model_type    = "ortho_mix_ecapa_emg"
    ...
    # ── training ──────────────────────────────────────────────────────────────
    # fit() computes normalisation from X_train only; augmentation is applied
    # only during training batches (model.train() mode).
    try:
        ...
        return {
            "test_subject":  test_subject,
            "model_type":    "ortho_mix_ecapa_emg",
            "approach":      APPROACH,
            "test_accuracy": None,
            "test_f1_macro": None,
            "error":         str(e),
    ...
    # Assemble flat test arrays using class_ids ordering from trainer so
    ...
    # evaluate_numpy() calls model.eval() internally:
    #   • BatchNorm uses running stats from training (frozen)
    #   • OrthoMixECAPAEmg.training = False → no augmentation applied
    #   • Applies training-stats normalisation (mean_c, std_c from fit())
    # No test-subject adaptation of any kind.
...
def main():
    ...
    _parser.add_argument("--mix-epsilon", type=float, default=MIX_EPSILON,
                         help=f"Orthogonal mixing epsilon (default {MIX_EPSILON})")
    _parser.add_argument("--mix-prob", type=float, default=MIX_PROB,
                         help=f"Per-batch mixing probability (default {MIX_PROB})")
    ...
    mix_epsilon = _args.mix_epsilon
    mix_prob    = _args.mix_prob
    ...
    print(
        f"Hypothesis  : ECAPA-TDNN + orthogonal channel-mixing augmentation\n"
        f"              (A = exp(εS) ∈ SO(C), applied to training batches only)"
    )
    ...
    print(f"Ortho-mix   : epsilon={mix_epsilon}, prob={mix_prob}")
    ...
    for test_subject in ALL_SUBJECTS:
        ...
        fold_dir = OUTPUT_ROOT / "ortho_mix_ecapa_emg" / f"test_{test_subject}"
    ...
    if valid:
        ...
        print("\n" + "=" * 80)
        print("LOSO SUMMARY — OrthoMixECAPAEmg")
