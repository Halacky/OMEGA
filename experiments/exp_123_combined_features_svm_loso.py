"""
Experiment 123: Combined Feature Sets → SVM for LOSO EMG Classification

Hypothesis:
    Best SVM result = 37.89% (exp_116 B: Handcrafted + Entropy → SVM-RBF).
    MFCC flat features carry spectral information not in handcrafted TD features.
    Combining all three feature types may push beyond 38%.

    Feature types:
      - Handcrafted (HC): 96 features — MAV, RMS, WL, ZCR, skew, kurt, spectral entropy, band powers
      - Entropy (ENT): 112 features — Rényi entropy (α=0.5,1,2,3), H_norm, Q_JS, C in time+freq domains
      - MFCC flat: variable — 6 statistics per MFCC coefficient per channel

What is tested:
      A) HC + Entropy + MFCC(fmax=500) → SVM-RBF  (all three, narrow MFCC)
      B) HC + Entropy + MFCC(fmax=1000) → SVM-RBF  (all three, fullband MFCC)
      C) Entropy + MFCC(fmax=1000) → SVM-RBF       (no HC, test if HC redundant)
      D) HC + Entropy → SVM-RBF                     (baseline reproduction of exp_116 B)

Run:
    python experiments/exp_123_combined_features_svm_loso.py --ci
    python experiments/exp_123_combined_features_svm_loso.py --full
"""
...
EXPERIMENT_NAME = "exp_123_combined_features_svm"
EXERCISES       = ["E1"]
USE_IMPROVED    = True
MAX_GESTURES    = 10
...
CONFIGS = {
    "A": {
        "use_hc": True, "use_entropy": True, "use_mfcc": True, "mfcc_fmax": 500.0,
        "label": "HC + Entropy + MFCC(500) → SVM-RBF",
    },
    "B": {
        "use_hc": True, "use_entropy": True, "use_mfcc": True, "mfcc_fmax": 1000.0,
        "label": "HC + Entropy + MFCC(1000) → SVM-RBF",
    },
    "C": {
        "use_hc": False, "use_entropy": True, "use_mfcc": True, "mfcc_fmax": 1000.0,
        "label": "Entropy + MFCC(1000) → SVM-RBF",
    },
    "D": {
        "use_hc": True, "use_entropy": True, "use_mfcc": False, "mfcc_fmax": 500.0,
        "label": "HC + Entropy → SVM-RBF (baseline)",
    },
}
...
def run_single_loso_fold(
    ...
    if cfg["use_hc"]:
        ...
        parts_tr.append(hc.transform(X_train_std))
        parts_te.append(hc.transform(X_test_std))
        logger.info(f"  HC: {parts_tr[-1].shape[1]} features")

    if cfg["use_entropy"]:
        ent = EntropyComplexityExtractor(
            sampling_rate=proc_cfg.sampling_rate, order=3, delay=1,
        )
        parts_tr.append(ent.transform(X_train_std))
        parts_te.append(ent.transform(X_test_std))
        logger.info(f"  Entropy: {parts_tr[-1].shape[1]} features")

    if cfg["use_mfcc"]:
        mfcc_ext = EMGMFCCExtractor(
            sampling_rate=proc_cfg.sampling_rate, n_mfcc=13, n_mels=26,
            fmin=20.0, fmax=cfg["mfcc_fmax"], use_deltas=True,
        )
        parts_tr.append(mfcc_ext.transform(X_train_std))
        parts_te.append(mfcc_ext.transform(X_test_std))
        logger.info(f"  MFCC flat (fmax={cfg['mfcc_fmax']}): {parts_tr[-1].shape[1]} features")

    feat_train = np.nan_to_num(np.concatenate(parts_tr, axis=1))
    feat_test = np.nan_to_num(np.concatenate(parts_te, axis=1))
    ...
    if feat_train.shape[1] > 200:
        ...
        logger.info(f"PCA: {feat_train.shape[1] + (feat_train.shape[1] - n_comp)} → {n_comp}")
...
def main():
    ...
    print(f"Hypothesis : Combined feature sets (HC + Entropy + MFCC) → SVM-RBF")
    ...
    for config_key, results in all_results.items():
        ...
        if valid:
            ...
            print(f"    vs exp_116 B (37.89%): {np.mean(accs)*100 - 37.89:+.2f}pp")
    ...
    try:
        ...
        if best_config:
            ...
            mark_hypothesis_verified("H_COMBINED_SVM", {
                "best_config": best_config,
                "mean_accuracy": float(np.mean([r["test_accuracy"] for r in valid])),
                "mean_f1_macro": float(best_f1),
            }, experiment_name=EXPERIMENT_NAME)
        else:
            mark_hypothesis_failed("H_COMBINED_SVM", "All configs failed")
    except ImportError:
        pass
