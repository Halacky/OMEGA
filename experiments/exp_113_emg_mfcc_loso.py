"""
Experiment 113: EMG-adapted MFCC Features for LOSO Gesture Classification

Hypothesis:
    MFCC (Mel-Frequency Cepstral Coefficients), the de facto standard in speech
    recognition, can be adapted for sEMG signals by shifting the mel filterbank
    to the EMG-informative frequency range [20, 500] Hz.  The cepstral
    representation decorrelates spectral features and provides a compact,
    robust input for both ML (SVM-RBF) and deep (2D CNN) classifiers.

What is tested:
    Four configurations are evaluated head-to-head:
      A) MFCC flat features (13 MFCC + Δ + ΔΔ = 39 coeff, 6 statistics) → SVM-RBF
      B) MFCC flat features → Random Forest
      C) MFCC spectrogram (39 × T_frames × C) → 2D CNN with depthwise-separable conv
      D) MFCC spectrogram → 2D CNN (no deltas, 13 coeff only)

LOSO protocol:
    ┌─────────────────────────────────────────────────────────────────────���────┐
    │  For each fold (test_subject = one of N subjects):                      │
    │    1. Load windowed EMG for all subjects.                               │
    │    2. Channel mean/std from training subjects ONLY.                     │
    │    3. Compute MFCC from standardized windows (deterministic, no params).│
    │    4. Train classifier on training MFCC features/spectrograms.          │
    │    5. Evaluate on frozen test subject.                                  │
    └─────��──────────────────────────────────────��─────────────────────────────┘

Run examples:
    # Quick CI (5 subjects, all 4 configs):
    python experiments/exp_113_emg_mfcc_loso.py --ci

    # Single config (faster):
    python experiments/exp_113_emg_mfcc_loso.py --ci --config A

    # Full 20-subject:
    python experiments/exp_113_emg_mfcc_loso.py --full
"""
...
EXPERIMENT_NAME = "exp_113_emg_mfcc"
EXERCISES       = ["E1",]
USE_IMPROVED    = True
MAX_GESTURES    = 10

# MFCC hyperparameters (adapted for EMG)
MFCC_N_COEFF   = 13       # cepstral coefficients
MFCC_N_MELS    = 26       # mel filterbank channels
MFCC_FMIN      = 20.0     # Hz — EMG lower bound
MFCC_FMAX      = 500.0    # Hz — EMG upper bound (H1: above this = noise)

# Experiment configurations
CONFIGS = {
    "A": {"mode": "ml",   "ml_classifier": "svm_rbf",  "use_deltas": True,  "label": "MFCC+Δ+ΔΔ → SVM-RBF"},
    "B": {"mode": "ml",   "ml_classifier": "rf",       "use_deltas": True,  "label": "MFCC+Δ+ΔΔ → RF"},
    "C": {"mode": "deep", "ml_classifier": None,       "use_deltas": True,  "label": "MFCC+Δ+ΔΔ → 2D-CNN"},
    "D": {"mode": "deep", "ml_classifier": None,       "use_deltas": False, "label": "MFCC (no Δ) → 2D-CNN"},
}
...
def run_single_loso_fold(
    ...
    """Execute one LOSO fold for a given MFCC configuration."""
...
def main():
    ...
    print(f"Hypothesis : EMG-adapted MFCC features for cross-subject gesture recognition")
    print(f"Subjects   : {ALL_SUBJECTS}")
    print(f"Configs    : {configs_to_run}")
    print(f"MFCC       : n_mfcc={MFCC_N_COEFF}, n_mels={MFCC_N_MELS}, "
          f"freq=[{MFCC_FMIN}, {MFCC_FMAX}] Hz")
    ...
    try:
        ...
        if best_config is not None:
            ...
            mark_hypothesis_verified("H_MFCC_EMG", metrics,
                                     experiment_name=EXPERIMENT_NAME)
        else:
            mark_hypothesis_failed("H_MFCC_EMG", "All configurations failed")
    except ImportError:
        pass
