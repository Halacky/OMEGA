"""
Experiment 29: Spectral Transformer for EMG Gesture Classification (LOSO)

Hypothesis H3: Self-attention over frequency bands + channels > CNN-RNN.
STFT transforms raw EMG to time-frequency domain. Axial attention
(time + channel) with relative positional encoding models long-range
dependencies that CNNs miss. EMG is a quasi-spectral signal.

Architecture: STFT → Conv2d spectral embedding → axial attention (time + channel) → classifier
Baseline comparison: exp_1 (CNN-GRU-Attention, deep_raw), exp_27 (MoE CNN-GRU-Attention)
"""
...
register_model("spectral_transformer", SpectralTransformer)
...
def run_single_loso_fold(
    ...
    meta = {
        ...
        "model_type": model_type,
        "exercises": exercises,
        "hypothesis": "H3: Spectral self-attention > CNN-RNN",
        "config": {
...
def main():
    ...
    train_cfg = TrainingConfig(
        ...
        weight_decay=1e-4,
        dropout=0.3,
        early_stopping_patience=10,  # more patience for transformer convergence
    ...
    global_logger.info("Hypothesis: H3 — Spectral self-attention > CNN-RNN")
