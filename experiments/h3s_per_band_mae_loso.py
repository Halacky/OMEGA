#!/usr/bin/env python3
"""
H3s: Per-Band MAE Masking Strategy Ablation (Confirmatory, Family B).

Tests which frequency-band masking strategy produces the best SSL encoder
for cross-subject sEMG gesture recognition.  All variants use FreqAwareMAE
with different mask_strategy values.

Variants
────────
  random_mask: FreqAwareMAE mask_strategy="random", n_mask=2
  cv_biased:   FreqAwareMAE mask_strategy="cv_biased", n_mask=2
  high_only:   FreqAwareMAE mask_strategy="high_only", n_mask=2
  low_only:    FreqAwareMAE mask_strategy="low_only", n_mask=2

Protocol (per LOSO fold)
────────────────────────
  Phase 1 — SSL Pretrain: 30 epochs, batch=1024, lr=1e-3, cosine decay
  Phase 2 — Fine-tune: probe (5 ep) + full (40 ep, lr=1e-4)
  Phase 3 — Evaluate: accuracy, macro-F1

Statistical design
──────────────────
  Family B (k=2): cv_biased vs random_mask, high_only vs random_mask
  Wilcoxon signed-rank + Holm-Bonferroni correction (alpha=0.05)

Usage
─────
  python experiments/h3s_per_band_mae_loso.py                             # CI
  python experiments/h3s_per_band_mae_loso.py --full                       # 20 subj
  python experiments/h3s_per_band_mae_loso.py --gpu 0 --folds 0,1,2,3,4
  python experiments/h3s_per_band_mae_loso.py --variants random_mask cv_biased
"""
...
VARIANT_NAMES = ["random_mask", "cv_biased", "high_only", "low_only"]
VARIANT_LABELS = {
    "random_mask": "MAE random mask (n=2)",
    "cv_biased": "MAE CV-biased mask (n=2)",
    "high_only": "MAE high-only mask (n=2)",
    "low_only": "MAE low-only mask (n=2)",
}

VARIANT_MASK_STRATEGY = {
    "random_mask": "random",
    "cv_biased": "cv_biased",
    "high_only": "high_only",
    "low_only": "low_only",
...
EXPERIMENT_NAME = "h3s_per_band_mae_loso"
...
def cosine_schedule(epoch: int, total_epochs: int, base_lr: float, warmup: int = 3) -> float:
    if epoch < warmup:
...
# ═════════════════════════════════════════════════════════════════════
#  Data Loading (once for all variants)
# ═════════════════════════════════════════════════════════════════════
...
def load_data_once(
    ...
    # Reorder subjects to match loaded data
    subjects = [s for s in subjects if s in subj_arrays]
    return subj_arrays, num_classes, subjects


# ═════════════════════════════════════════════════════════════════════
#  SSL Pretraining (MAE with variant-specific masking)
# ═════════════════════════════════════════════════════════════════════
...
def ssl_pretrain_mae(
    ...
    mask_strategy = VARIANT_MASK_STRATEGY[variant]
    n_total = len(X_train) + len(X_val)
    n_batches_est = max(1, n_total // SSL_BATCH)
    logger.info(
        f"  [MAE-{variant}] Starting SSL pretraining: {n_total} windows, "
        f"~{n_batches_est} batches/ep, {SSL_EPOCHS} epochs (mask={mask_strategy})"
    )
    ...
    for epoch in range(SSL_EPOCHS):
        ...
        if epoch == 0 or (epoch + 1) % 5 == 0:
            ...
            logger.info(
                f"    MAE-{variant} ep {epoch+1}/{SSL_EPOCHS}  "
                f"loss={avg:.4f}  lr={lr:.6f}"
            )

    logger.info(f"  [MAE-{variant}] Pretraining complete.")
    return encoder
...
def finetune(
    ...
    logger.info(
        f"  [Finetune] {len(X_train)} train, {len(X_val)} val, "
    ...
    probe_optimizer = torch.optim.Adam(model.classifier.parameters(), lr=PROBE_LR)
    ...
    for epoch in range(FT_EPOCHS):
        ...
        if epoch == 0 or (epoch + 1) % 5 == 0 or patience_counter >= FT_PATIENCE:
            avg = epoch_loss / max(n_batches, 1)
            logger.info(
                f"    FT ep {epoch+1}/{FT_EPOCHS}  train={avg:.4f}  "
                f"val={val_loss:.4f}  pat={patience_counter}/{FT_PATIENCE}{improved}"
            )
...
def train_one_fold(
    ...
    encoder = ssl_pretrain_mae(variant, X_train, X_val, device, logger)
    model = finetune(
...
# ═════════════════════════════════════════════════════════════════════
#  LOSO Loop (with incremental fold saving)
# ═════════════════════════════════════════════════════════════════════
...
def statistical_comparison(
    ...
    comparisons = [
        ("cv_biased", "random_mask"),
        ("high_only", "random_mask"),
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="H3s: Per-Band MAE masking ablation")
    ...
    logger.info(f"H3s: Per-Band MAE Masking Strategy Ablation")
    ...
    try:
        ...
        baseline_f1 = all_results.get("random_mask", {}).get("mean_f1_macro", 0)
        if best_variant != "random_mask" and all_results[best_variant]["mean_f1_macro"] > baseline_f1:
            mark_hypothesis_verified("H3s", {
                "best_variant": best_variant,
                "mean_f1_macro": all_results[best_variant]["mean_f1_macro"],
                "delta_vs_random": all_results[best_variant]["mean_f1_macro"] - baseline_f1,
            }, experiment_name=EXPERIMENT_NAME)
        else:
            mark_hypothesis_failed("H3s", "No masking strategy significantly improved over random")
    except ImportError:
        pass
