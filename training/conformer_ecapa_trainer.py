"""
Trainer for ConformerECAPA (Experiment 70).

Instantiates and trains the ConformerECAPA model (Conformer blocks +
SE-Res2Net TDNN backbone) following the same LOSO-clean training pipeline
as ECAPATDNNTrainer (exp_62).

LOSO data-leakage checklist
────────────────────────────
  ✓ Channel mean/std computed from X_train ONLY — never from val or test.
  ✓ Training stats applied unchanged to val and test (no leakage direction).
  ✓ Test subject flows in through evaluate_numpy() only — never touches fit().
  ✓ model.eval() at inference: BatchNorm uses training running statistics;
    LayerNorm has no running statistics; DropPath is disabled.
  ✓ No per-subject centering, no adaptive normalisation at test time.

Differences from ECAPATDNNTrainer (exp_62)
──────────────────────────────────────────
  • Model class: ConformerECAPA (not ECAPATDNNEmg).
  • Extra hyper-parameters: num_conformer, conformer_heads, dw_kernel,
    max_rel_dist, attn_dropout, drop_path_prob, use_mixstyle.
  • Checkpoint filename: "conformer_ecapa_emg.pt".
  • Log / config keys updated to reflect the new architecture.
  All normalisation, early-stopping, and evaluation logic is identical to
  ECAPATDNNTrainer to ensure a fair, apples-to-apples LOSO comparison.
"""
...
from models.conformer_ecapa_emg import ConformerECAPA
...
class ConformerECAPA_Trainer(WindowClassifierTrainer):
    """
    Trainer for ConformerECAPA.

    All normalisation statistics are derived from training windows only.
    The model is trained jointly on pooled training subjects for each LOSO
    fold.  Test-subject inference uses frozen model parameters (model.eval())
    with no adaptation.
    ...
    Args:
        ...
        max_rel_dist:     Relative position bias clip distance (default 64).
        attn_dropout:     MHSA attention-weight dropout (default 0.1).
        drop_path_prob:   DropPath stochastic depth probability (default 0.1).
    ...
    def fit(self, splits: Dict) -> Dict:
        """
        Train ConformerECAPA on the LOSO training split.
        ...
        # ECAPATDNNEmg and ConformerECAPA both use channels-first (B, C, T).
        # _prepare_splits_arrays returns (N, T, C); detect by T > C heuristic
        # (always true for EMG: T=600 >> C=8).
        ...
        # LOSO invariant: mean and std are computed exclusively from X_train.
        # The same statistics are applied to val and test without re-computing.
        ...
        # ── 4. Build model ───────────────────────────────────────────────────
        model = ConformerECAPA(
            in_channels=in_channels,
            num_classes=num_classes,
            channels=self.channels,
            num_conformer=self.num_conformer,
            conformer_heads=self.conformer_heads,
        ...
        self.logger.info(
            f"ConformerECAPA: in_ch={in_channels}, classes={num_classes}, "
            f"C={self.channels}, N_conf={self.num_conformer}, "
            f"heads={self.conformer_heads}, dw_k={self.dw_kernel}, "
            f"scale={self.scale}, dilations={self.dilations}, "
            f"embed={self.embedding_dim} | total_params={total_params:,}"
        )
        ...
        for epoch in range(1, self.cfg.epochs + 1):
            ...
            for xb, yb in dl_train:
                ...
                loss.backward()
                # Gradient clipping: stabilises training when SE + MHSA interact.
        ...
        torch.save(
            ...
            self.output_dir / "conformer_ecapa_emg.pt",
        )
        self.logger.info(
            f"Checkpoint saved → {self.output_dir / 'conformer_ecapa_emg.pt'}"
        )
    ...
    def evaluate_numpy(
        ...
        No test-subject statistics are used — normalisation parameters are
        determined entirely by training data in fit().
        ...
        # Same heuristic as fit(): T (dim=1) > C (dim=2) for EMG windows.
        ...
        # model.eval(): BatchNorm uses training running stats; DropPath disabled.
        self.model.eval()
        all_logits, all_y = [], []
        with torch.no_grad():
            for xb, yb in dl:
